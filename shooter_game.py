...
def restart_game():
    ...
    time.delay(500) # 3 sec.
 
    for i in range(6):
        monster = Enemy(img_ufo, randint(10, WIDTH - 100), -50, 100, 50, randint(1, 10))
        monsters.add(monster)
 
    for i in range(3):
        asteroid = Astetoid(img_asteroid, randint(10, WIDTH - 100), -50, 50, 50, randint(1, 10))
        asteroids.add(asteroid)

def god_mode():
    global score, lost, finish, life, ammo_indicator, player
    max_score = 4032742397890
    max_lost = 52342654354353453450
    life = 999999999999999999
    ammo_indicator.kill()
    ammo_indicator = AmmoIndicator(img_bullet, WIDTH - 10, HEIGHT - 10, 15, 20, 355)
    player.kill()
    player = Player(img_hero, 5, HEIGHT - 100, 60, 100, 10, 335)
